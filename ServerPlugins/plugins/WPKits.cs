...
{
    [Info("WPKits", "David", "1.2.2")]
    public class WPKits : RustPlugin
    {   
        [PluginReference]Plugin Kits, WelcomePanel, ImageLibrary, Notifications, ItemChsName;
        ...
        private string _Path_name = null;
        ...
        private void AddImage(string fileName)
        {
            ...
            {
                url = _Path_name + fileName;
            }
            ImageLibrary?.Call("AddImage", url, fileName, 0UL);
        }
        private void DownloadImages()
        {
            _Path_name = "file://" + Interface.Oxide.DataDirectory + Path.DirectorySeparatorChar;

            if (!string.IsNullOrEmpty(config.Backgroundimage))
            {
                AddImage(config.Backgroundimage);
        ...
        private void wpkits_import(ConsoleSystem.Arg arg)
        {
            ...
            if (args.Length == 1) 
            {
                ...
                if (!config.kits.ContainsKey(args[0]))
                {
                    ...
                    config.kits[args[0]].text = "可在配置文件中单独修改的描述\n剩余冷却: <b><color=#1175A5>{kitCooldown}</color></b>";
        ...
        private string GetKitCd(string _kitName) 
        {
            ...
            string cooldownFormated = string.Format("{0:D1}天{1:D2}时{2:D2}分{3:D2}秒", cooldownTS.Days, cooldownTS.Hours, cooldownTS.Minutes, cooldownTS.Seconds);
            if (_kitCdInt < 86400) cooldownFormated = string.Format("{0:D2}时{1:D2}分{2:D2}秒", cooldownTS.Hours, cooldownTS.Minutes, cooldownTS.Seconds);
        ...
        { 
            ...
            string cooldownFormated = string.Format("{0:D1}天 {1:D2}时{2:D2}分{3:D2}秒", cooldownTS.Days, cooldownTS.Hours, cooldownTS.Minutes, cooldownTS.Seconds);
            if (_currentCdInt < 86400) cooldownFormated = string.Format("{0:D2}时{1:D2}分{2:D2}秒", cooldownTS.Hours, cooldownTS.Minutes, cooldownTS.Seconds);
        ...
        private string KitItemList(string kitName)
        {
            ...
            {   
                ...
                //翻译接口 - 开始
                string strItemChsName = ItemChsName?.Call("ServerGetItemChsName", shortname) as string;
                if (string.IsNullOrEmpty(strItemChsName)) strItemChsName = shortname;
                //翻译接口 - 结束
                string amount = $"{item.SelectToken("Amount")}";
                var itemDef = ItemManager.FindItemDefinition(shortname);
                if (itemDef == null)
                    result += $"【{amount}x <错误的短名称>】   ";
                else
                    result += $"【{amount}x {strItemChsName}】   ";
                    //翻译接口 - 替换
            }
        ...
        {
            if (!ImageLibrary) return null;
            if ((bool)ImageLibrary.Call("HasImage", fileName, skin))
            {
                return (string)ImageLibrary.Call("GetImage", fileName, skin);
            }
            else
            {
                return (string)ImageLibrary.Call("GetImage", fileName, 0);
            }
        }
        ...
        {   
            ...
            string PicMian = GetImage(config.Backgroundimage);

            if (!string.IsNullOrEmpty(PicMian))
            {
                CUIClass.CreateImageB(ref _uiNotification, "main_panel", PicMian, "0 0", "1 1");
        ...
        public class CUIClass
        {
            ...
            public static void CreateImageB(ref CuiElementContainer _container, string _parent, string _image, string _anchorMin, string _anchorMax, float _fade = 0f)
            {
                _container.Add(new CuiElement
                {
                    Parent = _parent,
                    Components =
                        {
                            new CuiRawImageComponent { Png = _image, FadeIn = _fade},
                            new CuiRectTransformComponent { AnchorMin = _anchorMin, AnchorMax = _anchorMax }
                        }
                });
            }
        ...
        {   
            ...
            [JsonProperty(PropertyName = "背景图片")]
            ...
            public static Configuration CreateConfig()
            {
                ...
                {   
                    ...
                    Backgroundimage = "https://pic.qqtn.com/up/2018-12/15458904154447065.jpg",
                    
                    kits = new Dictionary<string, KitProperties>
                    {
                        {
                            "123",new KitProperties
                            {
                                desc = "123",
                                displayName ="123",
                                image = "",
